...
#ifdef simc_tree_SIDIS_cxx
simc_tree_SIDIS::simc_tree_SIDIS(TTree *tree) : fChain(0) 
{
...
simc_tree_SIDIS::~simc_tree_SIDIS()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t simc_tree_SIDIS::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t simc_tree_SIDIS::LoadTree(Long64_t entry)
{
...
void simc_tree_SIDIS::Init(TTree *tree)
{
...
Int_t simc_tree_SIDIS::Cut(Long64_t entry)
{
...
#endif // #ifdef simc_tree_SIDIS_cxx
